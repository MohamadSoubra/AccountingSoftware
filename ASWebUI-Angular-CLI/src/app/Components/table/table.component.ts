...
//Added changeDetection: ChangeDetectionStrategy.OnPush property in @component 
//because of ExpressionChangedAfterItHasBeenCheckedError (MatPaginator Bug)
//after changing the data in TableDataSource to DATA$ BehaviorSubject
//https://stackoverflow.com/questions/56004993/expressionchangedafterithasbeencheckederror-in-mat-table-when-there-is-ngif-in/56006747
//Tushar's Aswer
@Component({
...
export class TableComponent<T extends Identification> implements OnInit, AfterViewInit {
  ...
  delete(element) {
    ...
    let data: any = this.tableDataItems.DATA$.value.filter((el) => el != element);
